...
int main() {
    ...
    // --- Output initial best costs from stage 0 ---
    cout << fixed << setprecision(6);
    cout << "\nBest costs from Stage-0 nodes:\n";
    for (int k = 0; k < stage_count[0]; ++k) {
        int u = stage_start[0] + k;
        if (best_cost[u] >= INF / 2)
            cout << "Node " << u << ": unreachable\n";
        else
            cout << "Node " << u << ": cost = " << best_cost[u] << "\n";
    }

    // --- Function to print path ---
    auto print_path = [&](int src) {
...
// Enter number of live updates to edge costs (0 to finish): 2
// Enter edge update (u v multiplier): 3 4 2
// Enter edge update (u v multiplier): 0 2 0.5
// Enter a source node id (in stage 0) to print path, or -1 to skip: 0
...
// After updates, best costs from Stage-0 nodes:
// Node 0: cost = 6.000000
// Node 1: cost = 5.000000
