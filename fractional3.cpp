...
// Option A: Strict priority groups
double priorityStrictKnapsack(vector<Item>& items, double capacity, double& totalWeightCarried) {
    totalWeightCarried = 0.0;
    double totalValue = 0.0;

    // process priorities 1 (high) -> 2 -> 3
    for (int pri = 1; pri <= 3; ++pri) {
        // collect items of this priority
        vector<Item> group;
        for (const auto& it : items) if (it.priority == pri) group.push_back(it);

        if (group.empty()) continue;

        // within group, sort indivisible items by value/weight desc,
        // and divisible items by value/weight desc too.
        sort(group.begin(), group.end(), [](const Item& a, const Item& b) {
            return a.valuePerWeight() > b.valuePerWeight();
        });

        cout << "\nProcessing priority " << pri << " items:\n";

        // First attempt to take indivisible items greedily (if they fully fit).
        // This keeps the "don't split an indivisible item" behavior.
        ...
        // only after all divisible items & indivisible attempts within this priority are done,
